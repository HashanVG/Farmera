    ...
    <main className="min-h-screen pt-32 pb-16 px-6 relative bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=2064&auto=format&fit=crop')" }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {agents.map((officer, index) => {
            const style = getAgentStyle(officer.role);
            return (
              <div key={index} className={`bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 hover:-translate-y-2 transition-all duration-300 shadow-xl hover:${style.shadow} group relative overflow-hidden`}>
                <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${style.gradient} rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity`}></div>
                
                <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${style.gradient} flex items-center justify-center text-white mb-6 shadow-lg transform group-hover:scale-110 transition-transform`}>
                  {style.icon}
                </div>
          ...
          <div className="p-10 md:w-3/5">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                >
                  {agents.map(agent => (
                    <option key={agent.id} value={agent.role} className="text-gray-900">
